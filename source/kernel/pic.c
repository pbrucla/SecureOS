...
void PIC_remap(int offset1, int offset2)
{
    ...
    outb(ICW1_INIT | ICW1_ICW4,
         PIC1_COMMAND); // starts the initialization sequence (in cascade mode)
    io_wait();
    ...
    io_wait();
    outb(4, PIC1_DATA); // ICW3: tell Master PIC that there is a slave PIC at
                        // IRQ2 (0000 0100)
    io_wait();
    ...
    outb(ICW4_8086,
         PIC1_DATA); // ICW4: have the PICs use 8086 mode (and not 8080 mode)
    io_wait();
